...
class Generator(nn.Module):   
    def __init__(self, img_size=128, sty_dim=64, n_res=2, use_sn=False, use_attn=False):
        ...
        if use_attn:
            self.sp_att = nn.Conv2d(512, 2, kernel_size=1, padding=0)
    ...
    def sp_attention_v2(self, x):
        sp_w = self.sp_att(x)
        sp_w = sp_w.transpose(1, 2).transpose(2, 3)
        sp_w = self.softmax(sp_w)[:, :, :, 0].unsqueeze(1)
        left_w = 1 - sp_w.repeat(1, 512, 1, 1)
        x = left_w * x
        return x, sp_w

    def decode(self, cnt, sty, with_ref=True):
        adapt_params = self.mlp(sty)   # (10, 4992)
        self.adaptive_param_assign(adapt_params, self.decoder, with_ref=with_ref)
...
def assign_adain_params(adain_params, model, with_ref=True):
    ...
    for m in model.modules():
        if m.__class__.__name__ == "AdaIN2d":
            ...
            if with_ref:
                ...
                # 如果不要参考，那么就把bias置0，weight置1
                m.bias = torch.zeros(m.num_features * adain_params.size(0)).to(mean.device)
                m.weight = torch.ones(m.num_features * adain_params.size(0)).to(std.device)
            if adain_params.size(1) > 2*m.num_features:  # 后移
